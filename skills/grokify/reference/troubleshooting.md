...
inside a Linux VM isolated from the host, so a CLI installed on the host is not
on its PATH and cannot be put there. Installing one inside the VM does not help
either, and the VM is discarded when the session ends.

The plugin's host bridge is the route. It runs on the machine rather than in
the VM, so it reaches the CLI already installed and signed in there. When the
plugin is installed, the skill finds a tool ending in `grokify_rewrite` and uses
...
`home` against the machine. It also reports `grok_cli`, which is the answer to
most failures on its own.

If the tool is missing, Node 18 or later may not be on PATH - that is what runs
the bridge - or the server may still be starting, which takes a few seconds
after an install.

## Not authenticated (exit 126)

Run `grok login` and sign in, or `grok login --device-auth` on a headless box.
How you authenticate the CLI is between you and it; Grokify never reads or
stores credentials. Note that a login done only in a GUI shell will not
