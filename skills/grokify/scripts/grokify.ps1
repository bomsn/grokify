...
if ($run.Code -ne 0) {
    if ($run.Stderr -match '(?i)unauthori|not authenticated|invalid api key|missing api key|no api key|401|sign in|login required') {
        Write-Err 'grok is installed but not authenticated.'
        [Console]::Error.WriteLine("Run 'grok' once and sign in, or set XAI_API_KEY in this environment.")
